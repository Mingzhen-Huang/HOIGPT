...
def get_module_config(cfg, filepath="./configs"):
    ...
    for yaml in sorted(glob.glob(pjoin(filepath, '*', '*.yaml'))):
        nodes = os.path.relpath(yaml, filepath).replace('.yaml', '').replace(os.sep, '.')
        OmegaConf.update(cfg, nodes, OmegaConf.load(yaml), merge=True)
...
def resume_config(cfg: OmegaConf):
    ...
    if cfg.TRAIN.RESUME:
        ...
        if os.path.exists(resume):
            # Checkpoints
            cfg.TRAIN.PRETRAINED = resume if os.path.isfile(resume) else pjoin(resume, "checkpoints", "last.ckpt")
            if not os.path.isfile(cfg.TRAIN.PRETRAINED):
                raise FileNotFoundError(cfg.TRAIN.PRETRAINED)
            # Wandb
            ...
            if os.path.isdir(wandb_latest):
                ...
                if wandb_runs:
                    cfg.LOGGER.WANDB.params.id = wandb_runs[0].replace("run-", "").replace(".wandb", "")
        else:
...
def parse_args(phase="train"):
    ...
    group.add_argument(
        ...
        help="Override a config value; repeat for multiple values (e.g. TRAIN.END_EPOCH=10)",
    )
    ...
    cfg = OmegaConf.merge(cfg_exp, cfg_assets, OmegaConf.from_dotlist(params.overrides))
    if "MANO" in cfg:
        os.environ["HOIGPT_MANO_ROOT"] = str(cfg.MANO.ROOT)
