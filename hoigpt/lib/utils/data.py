...
from hoigpt.lib.utils.rot import (
    rot6d_to_axis_angle,
    rot6d_to_rotmat,
...
def process_hand_result(hand_layer, hand_params):
    ...
    return hand_vertices, hand_faces, out.joints
...
def process_obj_result(obj_verts, obj_params, dataset_name, obj_top_idx=None):
    ...
    if dataset_name == "arctic" and obj_params.shape[-1] == 10 and obj_top_idx is not None:
        ...
        obj_angle = obj_params[..., :1]
        quat_arti = axis_angle_to_quaternion(torch.FloatTensor([0, 0, -1]).to(obj_rot6d.device).view(1, 3)*obj_angle)
