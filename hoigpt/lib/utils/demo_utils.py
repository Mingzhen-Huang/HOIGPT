...
# from hoigpt.lib.networks.clip import encoded_text_normalized
from hoigpt.lib.utils.frame import get_valid_mask
from hoigpt.lib.utils.file import read_json
...
def proc_results(
    ...
    is_lhand, is_rhand, 
    dataset_name, obj_top_idx=None, return_joints=False
):
    # Object
    if obj_top_idx is not None and obj_top_idx[0] is not None:
        obj_top_idx = proc_long_torch_cuda(obj_top_idx).to(obj_verts.device)
    obj_verts_tf = process_obj_result(obj_verts, x_obj, dataset_name, obj_top_idx).float()
    if is_lhand:
        lhand_verts, lhand_faces, ljoints = process_hand_result(lhand_layer, x_lhand)
    else:
        lhand_verts, lhand_faces, rjoints = None, None
    if is_rhand:
        rhand_verts, rhand_faces, rjoints = process_hand_result(rhand_layer, x_rhand)
    else:
    ...
    if not return_joints:
        ...
        return obj_verts_tf, lhand_verts, lhand_faces, rhand_verts, rhand_faces, ljoints, rjoints
...
def get_object_hand_info(object_name, object_model, obj_root=None):
    if obj_root is None:
