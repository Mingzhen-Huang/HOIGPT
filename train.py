...
from hoigpt.callback import build_callbacks
from hoigpt.config import parse_args, instantiate_from_config
from hoigpt.data.build_data import build_data
from hoigpt.models.build_model import build_model
from hoigpt.utils.logger import create_logger
from hoigpt.utils.load_checkpoint import load_pretrained, load_pretrained_vae
...
def main():
    ...
    for loggerName in cfg.LOGGER.TYPE:
        if loggerName == 'tensorboard' or (loggerName == 'wandb' and cfg.LOGGER.WANDB.params.project):
            pl_logger = instantiate_from_config(
                cfg.LOGGER[loggerName.upper()])
    ...
    trainer = pl.Trainer(
        ...
        # precision='16',
        logger=pl_loggers or False,
        callbacks=callbacks,
        check_val_every_n_epoch=cfg.LOGGER.VAL_EVERY_STEPS,
        accelerator=cfg.ACCELERATOR,
        devices=list(cfg.DEVICE) if cfg.ACCELERATOR == 'gpu' else len(cfg.DEVICE),
